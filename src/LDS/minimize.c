...
Output_mini minimize(MAT w_in,double w1,double *Y,long Y_rows,double C,double C2,double S,MAT X,MAT Xu,MAT R,Opt2 opt2){
	...
	DD red=1;
	DD** temp;
	//  dont know  ----if length>0, S=['Linesearch']; else S=['Function evaluation']; end 
	LL i=0,is_failed=0;
	Output_Obj F=obj_fun(w_in,w1,Y,Y_rows,C,C2,S,X,Xu,R);
	i=(length<0)?i+1:i;
    MAT s;
    ...
    while(i<abs(length))
    {
			  ...
			  while(1)
			  {
					  	while(((F2.obj>F.obj+z1*RHO*d1)|(d2>-SIG*d1))&(M>0))
					  	{
					  		...
					  		if(F2.obj>F.obj)
					  		{
					  			z2 = z3 - (0.5*d3*z3*z3)/(d3*z3+F2.obj-f3);
					  		}
					  		else{
					  		    A = 6*(F2.obj-f3)/z3+3*(d2+d3);                                
	...
	if(success)
    {
         ...
         if ((max_val< opt2.tolX) | (-d2 < opt2.tolFun))break;
    }
    else
    {
         w_in.matrix = X0; 
